...
class TestConceptSetExpressionQueryBuilder(unittest.TestCase):
    ...
    def test_build_concept_set_sub_query_descendants(self):
        ...
        self.assertIn("FROM @vocabulary_database_schema.CONCEPT_ANCESTOR ca", query)
        self.assertIn("WHERE ca.ancestor_concept_id IN (1)", query)
    ...
    def test_build_concept_set_mapped_query(self):
        ...
        self.assertIn("INNER JOIN @vocabulary_database_schema.CONCEPT_RELATIONSHIP cr", query)
        self.assertIn("ON c.concept_id = cr.concept_id_2", query)
        self.assertIn("WHERE cr.relationship_id = 'Maps to'", query)
