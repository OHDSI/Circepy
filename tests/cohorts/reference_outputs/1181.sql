...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (42903427,920458,19050907,1507705,19061907,19086888,19017895,1518254,1555120,19055344,975125,19009116,1506270,19027186,1550557,1551099,19011127,977421,903963))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (42903427,920458,19050907,1507705,19061907,19086888,19017895,1518254,1555120,19055344,975125,19009116,1506270,19027186,1550557,1551099,19011127,977421,903963))
...
(
  ...
  select person_id, min(start_date) as era_start_date, DATEADD(day,-1 * 60, max(end_date)) as era_end_date
  from (
    ...
    from (
      ...
      from (
        select person_id, drug_exposure_start_date as start_date, DATEADD(day,(60 + 0),DRUG_EXPOSURE_END_DATE) as end_date
        FROM #drugTarget
      ) DT
    ) ST
  ) GR
