...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (756055,704059,704058))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
...
WHERE C.start_date > DATEFROMPARTS(2019, 12, 1)
...
select event_id, person_id, 
  case when DATEADD(day,3,start_date) > op_end_date then op_end_date else DATEADD(day,3,start_date) end as end_date
INTO #strategy_ends
