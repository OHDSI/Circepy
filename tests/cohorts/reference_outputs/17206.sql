...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (36715585,435224,440689,45766061,4119158))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (36715585,440689,45766061,4119158))
...
select event_id, person_id, 
  case when DATEADD(day,21,end_date) > op_end_date then op_end_date else DATEADD(day,21,end_date) end as end_date
INTO #strategy_ends
