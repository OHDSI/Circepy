...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4148615))

) I
) C UNION ALL 
SELECT 7 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (435224,440689,36674516,4148615,320073))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (435224,440689,36674516,320073))

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (36678497,434895,81097,4232178))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (36678497,4232178))
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (3010813,3000905,3001657,3046900,3003282))
...
WHERE (C.value_as_number >= 4.0000 and C.value_as_number <= 20.0000)
AND C.unit_concept_id in (8848,8961,9444)
...
select person_id, min(start_date) as start_date, DATEADD(day,-1 * 183, max(end_date)) as end_date
into #final_cohort
from (
  ...
  from (
    ...
    from (
      select person_id, start_date, DATEADD(day,183,end_date) as end_date
      from #cohort_rows
    ) CR
  ) ST
) GR
