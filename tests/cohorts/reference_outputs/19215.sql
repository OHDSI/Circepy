...
UNION  select c.concept_id
  ...
  and (ca.ancestor_concept_id in (137977))

) I
) C UNION ALL 
SELECT 3 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (433603,437090,440218,437681,438869,433029,4173577,4071735,4071737,4048292,4071736,37167277,4048610,439137,435656,4251487,195064,4221399))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (433603,437090,440218,437681,438869,433029,4173577,4071735,4071737,4048292,4071736,37167277,4048610,439137,435656,4251487,195064,4221399))
...
WHERE YEAR(C.start_date) - P.year_of_birth = 0
