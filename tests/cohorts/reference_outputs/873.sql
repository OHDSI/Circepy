...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (2773930,2773932,2773934,2003806,2003803,2003805,2780485,2780486,2776775,2776776,2776777,2776778,2780477,2780478,2780479,2780480,2776783,2776972,2776973,2776974))
...
JOIN #Codesets cs on (po.procedure_concept_id = cs.concept_id and cs.codeset_id = 79)
) C
...
(
  select po.person_id,po.procedure_occurrence_id,po.procedure_concept_id,po.visit_occurrence_id,po.quantity,po.procedure_date as start_date, DATEADD(day,1,po.procedure_date) as end_date 
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
JOIN #Codesets cns on (po.procedure_source_concept_id = cns.concept_id and cns.codeset_id = 79)
) C
