...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (36686922))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (36686922))
UNION
select distinct cr.concept_id_1 as concept_id
FROM
(
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (37394108,4163737,4122252))

) I
) C UNION ALL 
SELECT 16 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4148212))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4148212))
UNION
select distinct cr.concept_id_1 as concept_id
FROM
(
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (36309671,4143091,4122252,4140156,3049944,40764963,37073281,3035833,4138900,4146128))
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (444429))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (444429))
UNION
select distinct cr.concept_id_1 as concept_id
FROM
(
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (40383887,3119749,45473162,45524708,44790710,1404390))
...
AND C.value_as_concept_id in (36309345,36310085,36309475,36310741,45878548,45881916,45876467,45881623,3459354,3474574,3437429,3470980,4123508,4126673,4125547,4126674,9192,3472281,9191,45496317)
...
select C.person_id, C.specimen_id as event_id, C.specimen_date as start_date, DATEADD(d,1,C.specimen_date) as end_date,
       CAST(NULL as bigint) as visit_occurrence_id, C.specimen_date as sort_date
from 
(
  select s.* 
  FROM @cdm_database_schema.SPECIMEN s
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 15)
) C
...
WHERE YEAR(C.start_date) - P.year_of_birth <= 50
-- End Condition Occurrence Criteria
...
(
  select s.* 
  FROM @cdm_database_schema.SPECIMEN s
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 19)
) C
...
(
  select s.* 
  FROM @cdm_database_schema.SPECIMEN s
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 20)
) C
