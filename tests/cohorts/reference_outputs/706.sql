...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4055672,37026454,2212731,37041470,45889638,37044890,44809199,4245261,37070720))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4055672,37026454,2212731,37041470,45889638,37044890,44809199,4245261,37070720))
...
WHERE C.value_as_number > 13.7000
AND C.unit_concept_id in (9212,4121007,8555)
