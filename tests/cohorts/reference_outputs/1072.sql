...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4122252,37394108,4163737))

) I
) C UNION ALL 
SELECT 13 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4122252,44808404,4046130))

) I
) C UNION ALL 
SELECT 15 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (35205780,35205781,440032))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (35205780,35205781,440032))
UNION
select distinct cr.concept_id_1 as concept_id
FROM
(
...
AND C.value_as_concept_id in (36309345,36310085,36309475,36310741,45878548,45881916,45876467,45881623,3459354,3474574,3437429,4126674,3470980,4125547,4126673,4123508,9192,3472281,45881796,9191,45884084)
...
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 1)
) C
...
WHERE C.value_as_concept_id in (9191,45884084)
-- End Measurement Criteria
...
WHERE C.value_as_concept_id in (4181412,45879438,45884084,9191)
-- End Observation Criteria
...
(
  select s.* 
  FROM @cdm_database_schema.SPECIMEN s
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 12)
) C
...
WHERE C.value_as_concept_id in (45884084,9191)
-- End Measurement Criteria
...
(
  select s.* 
  FROM @cdm_database_schema.SPECIMEN s
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 13)
) C
