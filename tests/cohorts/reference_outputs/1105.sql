...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (193688,4307981,45763575))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (193688,4307981,45763575))

) I
) C UNION ALL 
SELECT 1 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4133765))
...
WHERE C.value_as_concept_id in (9191,3448973,35941196,35941844,35941086,36307837,36309573,35919140)
...
WHERE C.value_as_concept_id in (36307837,36309573,9191,3448973,35941196,35941844,35941086,35919257,35919140)
-- End Measurement Criteria
