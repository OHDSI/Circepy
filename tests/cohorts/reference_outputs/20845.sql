...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (36712850,255848))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (36712850,255848))

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (45763750,259992,4154776,260041,255084,436632))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (45763750,259992,4154776,260041,255084))
...
select event_id, person_id, 
  case when DATEADD(day,14,start_date) > op_end_date then op_end_date else DATEADD(day,14,start_date) end as end_date
INTO #strategy_ends
...
select person_id, min(start_date) as start_date, DATEADD(day,-1 * 60, max(end_date)) as end_date
into #final_cohort
from (
  ...
  from (
    ...
    from (
      select person_id, start_date, DATEADD(day,60,end_date) as end_date
      from #cohort_rows
    ) CR
  ) ST
) GR
