...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (1717704,1703687,1703603))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (1717704,1703687,1703603))

) I
) C UNION ALL 
SELECT 2 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4227539,37209448,4045976,4043916,4347543,4310847,45757253,4205455,437489,443943,3655846,45770836,37209447,4101894,4347542,440329,436336,373412,4326661,440323,4266196,3654664))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4227539,37209448,4045976,4043916,4347543,4310847,45757253,4205455,437489,443943,3655846,45770836,37209447,4101894,4347542,440329,436336,373412,4326661,440323,4266196,3654664))

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (37018291,46273148,444429,4042204))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (37018291,46273148,444429,4042204))
...
select person_id, min(start_date) as start_date, DATEADD(day,-1 * 270, max(end_date)) as end_date
into #final_cohort
from (
  ...
  from (
    ...
    from (
      select person_id, start_date, DATEADD(day,270,end_date) as end_date
      from #cohort_rows
    ) CR
  ) ST
) GR
