...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (197223))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (197223))
...
select person_id, min(start_date) as start_date, DATEADD(day,-1 * 14, max(end_date)) as end_date
into #final_cohort
from (
  ...
  from (
    ...
    from (
      select person_id, start_date, DATEADD(day,14,end_date) as end_date
      from #cohort_rows
    ) CR
  ) ST
) GR
