### Cohort Entry Events

People enter the cohort when observing any of the following:

1. condition occurrences of 'Ambulance utilization'.

2. visit occurrences of 'Ambulance utilization'.

3. procedure occurrences of 'Ambulance utilization'.

4. observations of 'Ambulance utilization'.

### Cohort Exit

The cohort end date will be offset from index event's end date plus 0 days.

### Cohort Eras

Remaining events will be combined into cohort eras if they are within 1 days of each other.


