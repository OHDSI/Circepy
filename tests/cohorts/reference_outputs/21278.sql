...
) A on A.person_id = P.person_id  AND A.START_DATE >= DATEADD(day,56,P.START_DATE) AND A.START_DATE <= DATEADD(day,730,P.START_DATE) ) cc 
...
WHERE PR.specialty_concept_id in (38004491,45756818)
-- End Condition Occurrence Criteria
...
) A on A.person_id = P.person_id  AND A.START_DATE <= DATEADD(day,0,P.START_DATE) AND A.END_DATE >= DATEADD(day,0,P.END_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
...
WHERE E.start_date >= DATEFROMPARTS(2022, 1, 1)
GROUP BY e.person_id, e.event_id
...
) A on A.person_id = P.person_id  AND A.START_DATE <= DATEADD(day,-365,P.START_DATE) AND A.END_DATE >= DATEADD(day,1,P.START_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
