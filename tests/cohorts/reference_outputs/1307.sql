...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (2002027,2728555,2728556,2728557,2728410,2728412,2728546,2728548,2728540,2728542,2728561,2728562,2728563,2728567,2728568,2728569,2728552,2728554,2728407,2728409,2728543,2728545,2728413,2728539,2728558,2728559,2728560,2728564,2728565,2728566,2728549,2728551,2000061))
...
JOIN #Codesets cs on (po.procedure_concept_id = cs.concept_id and cs.codeset_id = 86)
) C
...
(
  select po.person_id,po.procedure_occurrence_id,po.procedure_concept_id,po.visit_occurrence_id,po.quantity,po.procedure_date as start_date, DATEADD(day,1,po.procedure_date) as end_date 
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
JOIN #Codesets cns on (po.procedure_source_concept_id = cns.concept_id and cns.codeset_id = 86)
) C
