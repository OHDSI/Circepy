...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4266196))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4266196))
UNION
select distinct cr.concept_id_1 as concept_id
FROM
(
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4206579))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4206579))
UNION
select distinct cr.concept_id_1 as concept_id
FROM
(
...
AND C.value_as_concept_id in (36309345,36310085,36309475,36310741,45878548,45881916,45876467,45881623,3459354,3474574,3437429,3470980,4123508,4126673,4125547,4126674,9192,3472281,45881796,9191,45496317)
...
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 9)
) C
...
WHERE YEAR(C.start_date) - P.year_of_birth >= 60
-- End Condition Occurrence Criteria
...
(
  select s.* 
  FROM @cdm_database_schema.SPECIMEN s
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 8)
) C
...
(
  select s.* 
  FROM @cdm_database_schema.SPECIMEN s
where s.specimen_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 7)
) C
