...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (46286608,439727,4308946,4083350,4013106,4226111,45772057,4186235,4281232,197494,4223031,44783623))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (46286608,439727,4083350,4013106,4226111,45772057,4186235,4281232,197494,4223031,44783623))
