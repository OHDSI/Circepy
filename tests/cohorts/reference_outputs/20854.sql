...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (3007461,3031586,3024929,4267147,3010834))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (3007461,3031586,3024929,4267147,3010834))
...
WHERE (C.value_as_number >= 151.0000 and C.value_as_number <= 600.0000)
