...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4144746,28396,4009785,4122936,4301410,444238))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4144746,28396,4009785,4122936,4301410,444238))

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4149681,25518,4121113,4098749,4125628,4296149,4252396,4262394,4121114,40484201))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4149681,25518,4121113,4098749,4125628,4296149,4252396,4262394,4121114,40484201))
...
) A on A.person_id = P.person_id  AND A.START_DATE <= DATEADD(day,60,P.START_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
...
select event_id, person_id, 
  case when DATEADD(day,99999,end_date) > op_end_date then op_end_date else DATEADD(day,99999,end_date) end as end_date
INTO #strategy_ends
