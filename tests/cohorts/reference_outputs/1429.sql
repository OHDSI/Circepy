...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (1133201))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (1133201))
...
(
  ...
  from (
    ...
    from (
      ...
      from (
        select person_id, drug_exposure_start_date as start_date, DATEADD(day,(0 + 30),DRUG_EXPOSURE_END_DATE) as end_date
        FROM #drugTarget
      ) DT
    ) ST
  ) GR
