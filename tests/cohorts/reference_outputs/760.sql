...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (35200139,1593700,1511348))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (35200139,1593700,1511348))
...
WHERE DATEDIFF(d,C.start_date, C.end_date) >= 30
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,-30,P.END_DATE) AND A.END_DATE >= DATEADD(day,30,P.START_DATE) AND A.END_DATE <= P.OP_END_DATE ) cc 
