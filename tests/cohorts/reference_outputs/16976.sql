...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (757688,800878,785788,703244,766814,735979,712615))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (757688,800878,785788,703244,766814,735979,712615))

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (755695))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (755695))
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (36878783,19032424,710062,713109,750982,19039883,797617,798834,716968,717607,19023846,19037989,738156,715259,715939,755695,751412,778268,19122204,781705,19091830,794147,19110751,19007737,19080226,725131,714684,19127550,721724,19129635,36427294,722031,733896,754270,19098514,19084693,766209,739138,1398039,19041910,703470,703547,705755,19006186,743670,40234834,19008261,44507700))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (36878783,19032424,710062,713109,750982,19039883,797617,798834,716968,717607,19023846,19037989,738156,715259,715939,755695,751412,778268,19122204,781705,19091830,794147,19110751,19007737,19080226,725131,714684,19127550,721724,19129635,36427294,722031,733896,754270,19098514,19084693,766209,739138,1398039,19041910,703470,703547,705755,19006186,743670,40234834,19008261,44507700))
...
UNION  select c.concept_id
  ...
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (1366610))

) I
) C UNION ALL 
SELECT 20 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (372608,37312036,37312035,37397762,4041685,37312031,37312030,4046087,35608576,37399020,4092747,45771254,4307791,380701,376095,42742415,42742416,42742417,4178618,37311665,4044051,45765480,45765477,4059191,4139421,44782763,4009647,37311890,37396752,4245766,4046091,37312577,37116464,375239,4182210,4043378,373179,435088,378726,44782710,4047744,4044049))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (372608,37397762,376095,42742415,42742416,42742417,37116464,4182210,44782710,4047744,4044049))

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (35305863,377788,372610))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (35305863,372610))
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,0,P.START_DATE) AND A.END_DATE >= DATEADD(day,-365,P.START_DATE) AND A.END_DATE <= P.OP_END_DATE ) cc 
...
JOIN (
  -- Begin Drug Era Criteria
select C.person_id, C.drug_era_id as event_id, C.start_date, C.end_date,
    CAST(NULL as bigint) as visit_occurrence_id,C.start_date as sort_date, C.drug_concept_id as domain_concept_id
from 
(
  select de.person_id,de.drug_era_id,de.drug_concept_id,de.drug_exposure_count,de.gap_days,de.drug_era_start_date as start_date, de.drug_era_end_date as end_date 
  FROM @cdm_database_schema.DRUG_ERA de
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,-1,P.START_DATE) AND A.END_DATE >= DATEADD(day,-365,P.START_DATE) AND A.END_DATE <= P.OP_END_DATE ) cc 
GROUP BY cc.person_id, cc.event_id
