...
class TestDrugExposureBuilder(unittest.TestCase):
    ...
    def test_includes_dose_unit_logic(self):
        ...
        self.assertIn(
            ...
            sql,
            "SQL should select dose_unit_concept_id when doseUnit criteria is present",
        )

        # Check Where Clause
        self.assertIn(
            "C.dose_unit_concept_id in (123)",
            sql,
            "SQL should filter by dose_unit_concept_id in WHERE clause",
        )

    def test_includes_lot_number_logic(self):
        # Create DrugExposure with lot_number criteria
        de = DrugExposure(lotNumber=TextFilter(text="LOT123", op="eq"), first=False)
        ...
        self.assertIn(
            "de.lot_number",
            sql,
            "SQL should select lot_number when lotNumber criteria is present",
        )
...
class TestConditionOccurrenceBuilder(unittest.TestCase):
    ...
    def test_includes_condition_status_logic(self):
        ...
        self.assertIn(
            ...
            sql,
            "SQL should select condition_status_concept_id when conditionStatus criteria is present",
        )

        # Check Where Clause
        self.assertIn(
            "C.condition_status_concept_id in (456)",
            sql,
            "SQL should filter by condition_status_concept_id",
        )
...
class TestProcedureOccurrenceBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        self.assertIn(
            ...
            sql,
            "SQL should select procedure_type_concept_id",
        )
        ...
        self.assertIn(
            ...
            sql,
            "Should join PERSON for gender check",
        )
        # VisitType -> Join VisitOccurrence
        self.assertIn(
            "JOIN @cdm_database_schema.VISIT_OCCURRENCE V",
            sql,
            "Should join VISIT_OCCURRENCE for visit type check",
        )
        # ProviderSpecialty -> Join Provider
        self.assertIn(
            "LEFT JOIN @cdm_database_schema.PROVIDER PR",
            sql,
            "Should join PROVIDER for specialty check",
        )

        # 3. Check Where Clauses
        self.assertIn(
            "C.procedure_type_concept_id in (10)",
            sql,
...
class TestMeasurementBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        self.assertIn(
            ...
            sql,
            "Should select measurement_type_concept_id",
        )
        ...
        self.assertIn(
            "JOIN @cdm_database_schema.VISIT_OCCURRENCE V",
            sql,
            "Should join VISIT_OCCURRENCE",
        )

        # 3. Check Where Clauses
        self.assertIn(
            "C.measurement_type_concept_id in (10)",
            sql,
...
class TestObservationBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        self.assertIn(
            "JOIN @cdm_database_schema.VISIT_OCCURRENCE V ",
            sql,
            "Should join VISIT_OCCURRENCE with alias V",
        )

        # 3. Check Where Clauses
        self.assertIn(
            "C.observation_type_concept_id in (10)",
            sql,
...
class TestDeviceExposureBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        self.assertTrue(
            "V.visit_concept_id IN (40)" in sql or "V.visit_concept_id in (40)" in sql,
            "Should filter visit type",
        )
...
class TestDeathBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        self.assertIn(
            "C.start_date > DATEFROMPARTS(2020, 1, 1)",
            sql,
            "Should filter occurrence_start_date",
        )
...
class TestConditionEraBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        self.assertIn(
            ...
            sql,
            "Should filter codeset inside subquery with Java-style formatting",
        )
        ...
        # 4. Check Ordinal
        self.assertIn(
            "row_number() over (PARTITION BY ce.person_id ORDER BY ce.condition_era_start_date, ce.condition_era_id) as ordinal",
            sql,
            "Should have ordinal window func",
        )
        self.assertIn(
            "row_number() over (PARTITION BY ce.person_id ORDER BY ce.condition_era_start_date, ce.condition_era_id) as ordinal",
            sql,
...
class TestDoseEraBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        # 4. Check Ordinal
        self.assertIn(
            "row_number() over (PARTITION BY de.person_id ORDER BY de.dose_era_start_date, de.dose_era_id) as ordinal",
            sql,
            "Should have ordinal window func",
        )
        self.assertIn("C.ordinal = 1", sql, "Should filter first ordinal")
...
class TestSpecimenBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        # codeset join logic
        self.assertIn(
            "JOIN #Codesets cs on (s.specimen_concept_id = cs.concept_id and cs.codeset_id = 1)",
            sql,
            "Should filter codeset via JOIN",
        )

        self.assertIn(
            "C.specimen_date > DATEFROMPARTS(2020, 1, 1)",
            sql,
            "Should filter occurrence_start_date",
        )
        self.assertIn("C.quantity > 5", sql, "Should filter quantity")
...
class TestVisitDetailBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        # Codeset join logic
        self.assertIn(
            "JOIN #Codesets cs on (vd.visit_detail_concept_id = cs.concept_id and cs.codeset_id = 1)",
            sql,
        )
        ...
        self.assertIn(
            "DATEDIFF(d,C.start_date, C.end_date) > 1",
            sql,
...
class TestPayerPlanPeriodBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        # 3. Check Where Clauses
        self.assertIn(
            "C.payer_concept_id in (SELECT concept_id from #Codesets where codeset_id = 1)",
            sql,
            "Should filter payer_concept",
        )
        self.assertIn(
            "C.plan_concept_id in (SELECT concept_id from #Codesets where codeset_id = 2)",
            sql,
            "Should filter plan_concept",
        )
        self.assertIn(
            "C.sponsor_concept_id in (SELECT concept_id from #Codesets where codeset_id = 3)",
            sql,
            "Should filter sponsor_concept",
        )
        self.assertIn(
            "C.stop_reason_concept_id in (SELECT concept_id from #Codesets where codeset_id = 4)",
            sql,
...
class TestObservationPeriodBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        self.assertIn(
            "DATEDIFF(d,C.start_date, C.end_date) > 365",
            sql,
            "Should filter period_length",
        )

        self.assertIn(
            "YEAR(C.start_date) - P.year_of_birth > 18",
            sql,
            "Should filter age_at_start",
        )
        ...
        self.assertIn(
            "C.start_date <= DATEFROMPARTS(2020, 1, 1) and C.end_date >= DATEFROMPARTS(2020, 1, 1)",
            sql,
            "Should filter user defined start",
        )
        self.assertIn(
            "C.start_date <= DATEFROMPARTS(2021, 1, 1) and C.end_date >= DATEFROMPARTS(2021, 1, 1)",
            sql,
            "Should filter user defined end",
        )
...
class TestLocationRegionBuilder(unittest.TestCase):
    ...
    def test_includes_full_logic(self):
        ...
        lr = LocationRegion(codeset_id=1)
        ...
        self.assertIn(
            ...
            sql,
            "Should select from LOCATION_HISTORY",
        )
        self.assertIn(
