...
class TestVisitOccurrenceSqlBuilderParity(unittest.TestCase):
    ...
    def test_get_table_column_for_criteria_column(self):
        ...
        self.assertEqual(
            self.builder.get_table_column_for_criteria_column(CriteriaColumn.DURATION),
    ...
    def test_get_criteria_sql_basic(self):
        ...
        self.assertIn(
            "C.person_id, C.visit_occurrence_id as event_id, C.start_date, C.end_date",
            sql,
        )
    ...
    def test_get_criteria_sql_with_codeset(self):
        ...
        sql = self.builder.get_criteria_sql(criteria)
        self.assertIn(
            "JOIN #Codesets cs on (vo.visit_concept_id = cs.concept_id and cs.codeset_id = 123)",
            sql,
        )
    ...
    def test_get_criteria_sql_with_visit_type(self):
        ...
        self.assertIn(
            "C.visit_type_concept_id in (select concept_id from #Codesets where codeset_id = 456)",
            sql,
        )
    ...
    def test_get_criteria_sql_with_provider_specialty(self):
        ...
        self.assertIn(
            "PR.specialty_concept_id in (select concept_id from #Codesets where codeset_id = 789)",
            sql,
        )
    ...
    def test_get_criteria_sql_with_place_of_service(self):
        ...
        self.assertIn(
            "CS.place_of_service_concept_id in (select concept_id from #Codesets where codeset_id = 101)",
            sql,
        )
    ...
    def test_get_criteria_sql_with_place_of_service_location(self):
        ...
        sql = self.builder.get_criteria_sql(criteria)
        self.assertIn(
            "JOIN @cdm_database_schema.LOCATION_HISTORY LH on LH.entity_id = C.care_site_id AND LH.domain_id = 'CARE_SITE'",
            sql,
        )
        self.assertIn(
        ...
        self.assertIn(
            "JOIN #Codesets cs on (LOC.region_concept_id = cs.concept_id and cs.codeset_id = 202)",
            sql,
        )
    ...
    def test_get_criteria_sql_with_first(self):
        ...
        sql = self.builder.get_criteria_sql(criteria)
        self.assertIn(
            ", row_number() over (PARTITION BY vo.person_id ORDER BY vo.visit_start_date, vo.visit_occurrence_id) as ordinal",
            sql,
        )
