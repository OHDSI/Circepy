...
from circe.capr import (
    ...
    condition_occurrence, drug_exposure, drug_era, measurement, procedure,
    observation, visit, visit_detail, device_exposure, specimen, death,
    observation_period, payer_plan_period, location_region,
    condition_era, dose_era,
    at_least, at_most, exactly, with_all, with_any,
    during_interval, event_starts, event_ends, continuous_observation,
    sensitive_disease_cohort, specific_disease_cohort,
    acute_disease_cohort, chronic_disease_cohort, new_user_drug_cohort
)
from circe.vocabulary import concept_set, descendants, mapped, exclude
...
class TestCriteriaFunctions:
    ...
    def test_with_any(self):
        group = with_any(
            at_least(1, condition_occurrence(1)),
            at_least(1, condition_occurrence(2))
        )
...
class TestTemplates:
    ...
    def test_new_user_drug_cohort(self):
        expr = new_user_drug_cohort(
            drug_concept_set_id=2,
            washout_days=365,
            title="New User Test"
        )
