...
# =============================================================================
# Markdown Generation Tests (Moved from test_markdown_parity.py)
# =============================================================================
...
def test_real_cohorts_summary(request):
    ...
    for cohort_name in cohort_files:
        ...
        # Markdown check
        md, md_error = get_generated_markdown(cohort_name)
        ...
        if md:
            ...
            ref_md = get_reference_markdown(cohort_name)
            if ref_md:
                md_comparison = compare_markdown_outputs(md, ref_md)
                if md_comparison["is_identical"]:
                    results["md_matches"] += 1
                    app_results[cohort_name]["md_match"] = True
                else:
                    md_issues = analyze_markdown_differences(md, ref_md)
                    results["failures"].append(
                        {
                            "cohort": cohort_name,
                            "type": "Markdown",
