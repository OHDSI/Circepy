...
class TestExitCriteriaCheck:
    ...
    def test_check_missing_drug_concept_set(self):
        ...
        try:
            ...
            expression = CohortExpression(end_strategy=strategy)
            check = ExitCriteriaCheck()
            warnings = check.check(expression)
    ...
    def test_check_valid_exit_criteria(self):
        ...
        expression = CohortExpression(end_strategy={"CustomEra": {"drugCodesetId": 0}})
        check = ExitCriteriaCheck()
        warnings = check.check(expression)
...
class TestRangeCheck:
    ...
    def test_check_valid_range(self):
        ...
        range_warnings = [w for w in warnings if "negative value" in w.to_message()]
...
class TestOcurrenceCheck:
    ...
    def test_check_valid_occurrence(self):
        ...
        occurrence_warnings = [w for w in warnings if "at least 0" in w.to_message()]
...
class TestComparisons:
    ...
    def test_compare_criteria(self):
        ...
        death1 = Death(codeset_id=1, death_type_exclude=False, first=True)
        death2 = Death(codeset_id=1, death_type_exclude=False, first=True)
