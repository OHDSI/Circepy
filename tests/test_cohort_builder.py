...
class TestContextManager:
    ...
    def test_context_manager_result_access_before_exit_raises(self):
        ...
        with (
            pytest.raises(RuntimeError, match="inside the context manager"),
            CohortBuilder("Error Test") as cohort,
        ):
            cohort.with_condition(1)
...
class TestCohortWithCriteria:
    ...
    def test_require_returns_criteria_state(self):
        ...
        result = CohortBuilder("Test").with_condition(1).require_drug(2, anytime_before=True)
...
class TestBuildCohortExpression:
    ...
    def test_cohort_with_exclusion(self):
        expr = CohortBuilder("With Exclusion").with_condition(1).exclude_drug(2, anytime_before=True).build()

        assert expr.inclusion_rules is not None

    def test_cohort_with_multiple_criteria(self):
        expr = (
            CohortBuilder("Multiple Criteria")
            .with_drug(2)
            .first_occurrence()
            .with_observation(prior_days=365)
            .min_age(18)
...
class TestQueryMethods:
    ...
    def test_within_days_before(self):
        result = CohortBuilder("Test").with_condition(1).require_drug(2, within_days_before=365)
    ...
    def test_within_days_after(self):
        result = CohortBuilder("Test").with_condition(1).require_drug(2, within_days_after=30)
    ...
    def test_anytime_before(self):
        result = CohortBuilder("Test").with_condition(1).exclude_drug(2, anytime_before=True)

        config = result._rules[0]["group"].criteria[0].query_config
        assert config.time_window.days_before == 99999

    def test_same_day(self):
        result = CohortBuilder("Test").with_condition(1).require_drug(2, same_day=True)
