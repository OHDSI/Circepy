"""
Tests for ConditionEraSqlBuilder

This module contains comprehensive tests for the ConditionEraSqlBuilder class,
...
class TestConditionEraSqlBuilder:
    """Test cases for ConditionEraSqlBuilder."""
    ...
    def test_get_default_columns(self):
        ...
        expected = {CriteriaColumn.START_DATE, CriteriaColumn.END_DATE}
    ...
    def test_embed_codeset_clause_without_codeset_id(self):
        ...
        criteria = ConditionEra(codeset_id=None)
        query = "SELECT * FROM table @codesetClause WHERE 1=1"
        
        result = self.builder.embed_codeset_clause(query, criteria)
        
        assert "@codesetClause" not in result
        assert "WHERE ce.condition_concept_id" not in result
    
    def test_embed_ordinal_expression_with_first_true(self):
        """Test embed_ordinal_expression with first=True."""
        criteria = ConditionEra(first=True)
    ...
    def test_embed_ordinal_expression_with_first_false(self):
        ...
        criteria = ConditionEra(first=False)
        query = "SELECT * @ordinalExpression FROM table"
        where_clauses = []
    ...
    def test_embed_ordinal_expression_with_first_none(self):
        ...
        criteria = ConditionEra(first=None)
        query = "SELECT * @ordinalExpression FROM table"
        where_clauses = []
    ...
    def test_resolve_select_clauses_without_date_adjustment(self):
        ...
        assert "ce.person_id" in result
        assert "ce.condition_era_id" in result
        assert "ce.condition_concept_id" in result
        assert "ce.condition_occurrence_count" in result
        assert "ce.condition_era_start_date as start_date, ce.condition_era_end_date as end_date" in result
    ...
    def test_resolve_select_clauses_with_date_adjustment(self):
        ...
        criteria = ConditionEra(date_adjustment=date_adjustment)
    ...
    def test_resolve_join_clauses_with_age_at_start(self):
        ...
        criteria = ConditionEra(age_at_start=NumericRange(op=">=", value=18))
    ...
    def test_resolve_join_clauses_with_age_at_end(self):
        ...
        criteria = ConditionEra(age_at_end=NumericRange(op="<=", value=65))
    ...
    def test_resolve_join_clauses_with_gender(self):
        ...
        criteria = ConditionEra(gender=[Concept(concept_id=8507)])
    ...
    def test_resolve_join_clauses_with_gender_cs(self):
        ...
        criteria = ConditionEra(gender_cs=ConceptSetSelection(codeset_id=123, is_exclusion=False))
    ...
    def test_resolve_where_clauses_with_era_start_date(self):
        ...
        criteria = ConditionEra(era_start_date=DateRange(op=">=", value="2020-01-01"))
    ...
    def test_resolve_where_clauses_with_era_end_date(self):
        ...
        criteria = ConditionEra(era_end_date=DateRange(op="<=", value="2023-12-31"))
    ...
    def test_resolve_where_clauses_with_occurrence_count(self):
        ...
        criteria = ConditionEra(occurrence_count=NumericRange(op=">=", value=2))
        
        result = self.builder.resolve_where_clauses(criteria)
        
        assert len(result) == 1
        assert "C.condition_occurrence_count" in result[0]
    
    def test_resolve_where_clauses_with_era_length(self):
        """Test resolve_where_clauses with era_length."""
        criteria = ConditionEra(era_length=NumericRange(op=">=", value=30))
    ...
    def test_resolve_where_clauses_with_gender(self):
        ...
        criteria = ConditionEra(gender=[Concept(concept_id=8507), Concept(concept_id=8532)])
    ...
    def test_resolve_where_clauses_with_multiple_conditions(self):
        ...
        assert any("C.condition_occurrence_count" in clause for clause in result)
    ...
    def test_get_criteria_sql_with_codeset_id(self):
        ...
        assert "WHERE ce.condition_concept_id in (SELECT concept_id from #Codesets where codeset_id = 123)" in result
    ...
    def test_comprehensive_integration_test(self):
        ...
        assert "WHERE ce.condition_concept_id in (SELECT concept_id from #Codesets where codeset_id = 456)" in result
        assert "row_number() over" in result
        assert "C.ordinal = 1" in result
        assert "JOIN @cdm_database_schema.PERSON P on C.person_id = P.person_id" in result
        ...
        assert "C.condition_occurrence_count" in result
        assert "DATEDIFF(d,C.start_date, C.end_date)" in result
        assert "YEAR(C.start_date) - P.year_of_birth" in result
