...
"""
    ...
    def get_criteria_sql_with_options(
        ...
        if criteria.waveform_channel_source_value:
            where_clauses.append(
                BuilderUtils.build_text_filter_clause(
                    "C.waveform_channel_source_value", criteria.waveform_channel_source_value
                )
            )
        ...
        if criteria.metadata_source_value:
            where_clauses.append(
                BuilderUtils.build_text_filter_clause(
                    "C.metadata_source_value", criteria.metadata_source_value
                )
            )
        ...
        if criteria.procedure_occurrence_id:
            where_clauses.append(
                BuilderUtils.build_numeric_range_clause(
                    "C.procedure_occurrence_id", criteria.procedure_occurrence_id
                )
            )
