...
from circe.capr.attrition import AttritionRule, AttritionRules
...
def date_offset_exit(days: int, from_field: str = "startDate") -> ExitStrategy:
    ...
    return exit_strategy(end_strategy="date_offset", offset_days=days, offset_field=from_field)


def era(
    era_days: int = 0, study_start_date: Optional[str] = None, study_end_date: Optional[str] = None
) -> CohortEra:
    """
    ...
    """
    return CohortEra(era_days=era_days, study_start_date=study_start_date, study_end_date=study_end_date)
...
def _convert_to_cohort_expression(composed: ComposedCohort) -> CohortExpression:
    ...
    if composed.attrition and composed.attrition.rules:
        inclusion_rules = [_build_inclusion_rule(rule) for rule in composed.attrition.rules]
    ...
    if composed.exit:
        ...
        if composed.exit.censor_events:
            censoring_criteria = [_query_to_criteria(q) for q in composed.exit.censor_events]

    # Build collapse settings from era
    collapse_settings = None
    if composed.era:
        collapse_settings = CollapseSettings(era_pad=composed.era.era_days)
...
def _build_inclusion_rule(rule: AttritionRule) -> InclusionRule:
    ...
    return InclusionRule(name=rule.name, description=rule.description, expression=expression)
...
def _build_criteria_group(group: CriteriaGroup):
    ...
    return CirceCriteriaGroup(type=group.group_type, criteria_list=criteria_list)
...
def _build_correlated_criteria(criteria: Criteria) -> CorelatedCriteria:
    ...
    if criteria.aperture and criteria.aperture.start_window:
        ...
        start_window = Window(
            ...
            start=WindowBound(coeff=-1, days=interval.start),
            end=WindowBound(coeff=1, days=interval.end),
        )
...
def _build_end_strategy(exit_strat: ExitStrategy):
    ...
    if exit_strat.strategy_type == "date_offset":
        return DateOffsetStrategy(date_field=exit_strat.offset_field, offset=exit_strat.offset_days)
    elif exit_strat.strategy_type == "custom_era":
        return CustomEraStrategy(
            drug_codeset_id=exit_strat.drug_codeset_id, gap_days=exit_strat.custom_era_gap_days, offset=0
        )
