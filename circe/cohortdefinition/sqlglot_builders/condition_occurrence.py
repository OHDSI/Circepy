...
class ConditionOccurrenceGlotBuilder(SqlGlotCriteriaBuilder[ConditionOccurrence]):
    def build_select(self, criteria: ConditionOccurrence) -> sge.Select:
        inner = sge.Select()
        cols = [
            column_ref("co", "person_id"),
            column_ref("co", "condition_occurrence_id"),
            column_ref("co", "condition_concept_id"),
            column_ref("co", "visit_occurrence_id"),
        ]

        if criteria.condition_type is not None and len(criteria.condition_type) > 0:
            cols.append(column_ref("co", "condition_type_concept_id"))
        if criteria.condition_type_cs is not None:
            cols.append(column_ref("co", "condition_type_concept_id"))
        if criteria.stop_reason is not None:
            cols.append(column_ref("co", "stop_reason"))
        if criteria.provider_specialty is not None and len(criteria.provider_specialty) > 0:
            cols.append(column_ref("co", "provider_id"))
        ...
        if criteria.condition_status is not None and len(criteria.condition_status) > 0:
            cols.append(column_ref("co", "condition_status_concept_id"))
        if criteria.condition_status_cs is not None:
            cols.append(column_ref("co", "condition_status_concept_id"))

        if criteria.date_adjustment is not None:
            start_col = (
                column_ref("co", "condition_start_date")
                if criteria.date_adjustment.start_with == "start_date"
                else coalesce(
                    column_ref("co", "condition_end_date"),
                    date_add("day", 1, column_ref("co", "condition_start_date")),
                )
            )
            end_col = (
                column_ref("co", "condition_start_date")
            ...
            start_date_expr = column_ref("co", "condition_start_date")
        ...
        inner = inner.select(*cols).from_(sge.Table(this="CONDITION_OCCURRENCE", alias="co"))

        if criteria.codeset_id is not None:
            cs_table = sge.Table(this="#Codesets", alias="cs")
            cs_on = sge.And(
                this=sge.EQ(
                    this=column_ref("co", "condition_concept_id"), expression=column_ref("cs", "concept_id")
                ),
                expression=sge.EQ(
        ...
        if criteria.condition_source_concept is not None:
            ...
            cns_on = sge.And(
                this=sge.EQ(
                    this=column_ref("co", "condition_source_concept_id"),
                    expression=column_ref("cns", "concept_id"),
                ),
                expression=sge.EQ(
                    this=column_ref("cns", "codeset_id"),
                    expression=sge.Literal.number(criteria.condition_source_concept),
                ),
            )
        ...
        outer_cols = [
            ...
            alias_expr(column_ref("C", "condition_occurrence_id"), "event_id"),
        ...
        if criteria.condition_type is not None and len(criteria.condition_type) > 0:
            ...
            if concept_ids:
                wheres.append(
                    build_in_clause(
                        column_ref("C", "condition_type_concept_id"),
                        concept_ids,
        ...
        if criteria.condition_type_cs is not None:
            clause = codeset_in(
                column_ref("C", "condition_type_concept_id"),
                criteria.condition_type_cs.codeset_id,
                exclude=criteria.condition_type_cs.is_exclusion,
            )
            if clause is not None:
                wheres.append(clause)
        ...
        if criteria.condition_status is not None and len(criteria.condition_status) > 0:
            ...
            if concept_ids:
                wheres.append(build_in_clause(column_ref("C", "condition_status_concept_id"), concept_ids))

        if criteria.condition_status_cs is not None:
            clause = codeset_in(
                column_ref("C", "condition_status_concept_id"),
                criteria.condition_status_cs.codeset_id,
                exclude=criteria.condition_status_cs.is_exclusion,
            )
            if clause is not None:
                wheres.append(clause)
