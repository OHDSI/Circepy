...
"""
    ...
    def embed_ordinal_expression(
        ...
        if criteria.first:
            ...
            query = query.replace(
                "@ordinalExpression",
                ", row_number() over (PARTITION BY m.person_id ORDER BY m.measurement_date, m.measurement_id) as ordinal",
            )
        else:
            ...
            select_cols.append(
                BuilderUtils.get_date_adjustment_expression(
                    ...
                    (
                        ...
                        else "DATEADD(day,1,m.measurement_date)"
                    ),
                    (
                        "m.measurement_date"
    ...
    def resolve_where_clauses(
        ...
        if criteria.operator_cs:
            where_clauses.append(
                BuilderUtils.get_codeset_in_expression(
                    criteria.operator_cs.codeset_id, "C.operator_concept_id"
                )
            )
        ...
        if criteria.range_low_ratio:
            where_clauses.append(
                BuilderUtils.build_numeric_range_clause(
                    "(C.value_as_number / NULLIF(C.range_low, 0))",
                    criteria.range_low_ratio,
                    ".4f",
                )
            )
        ...
        if criteria.range_high_ratio:
            where_clauses.append(
                BuilderUtils.build_numeric_range_clause(
                    "(C.value_as_number / NULLIF(C.range_high, 0))",
                    criteria.range_high_ratio,
                    ".4f",
                )
            )
