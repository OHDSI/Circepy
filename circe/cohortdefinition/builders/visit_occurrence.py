...
"""
    ...
    def resolve_select_clauses(
        ...
        if criteria.date_adjustment:
            ...
            select_cols.append(
                BuilderUtils.get_date_adjustment_expression(criteria.date_adjustment, start_col, end_col)
            )
        else:
    ...
    def resolve_where_clauses(
        ...
        if criteria.occurrence_end_date:
            where_clauses.append(
                BuilderUtils.build_date_range_clause("C.end_date", criteria.occurrence_end_date)
            )
    ...
    def _add_filtering_by_care_site_location_region(self, join_clauses: list[str], codeset_id: int):
        ...
        join_clauses.append(
            BuilderUtils.get_codeset_join_expression(codeset_id, "LOC.region_concept_id", None, None)
        )
    ...
    def _get_location_history_join(self, alias: str, domain: str, entity_id_field: str) -> str:
        ...
        return (
            ...
            + domain
            + "' "
            + "AND C.visit_start_date >= "
            + alias
            + ".start_date "
            + "AND C.visit_end_date <= ISNULL("
            + alias
            + ".end_date, DATEFROMPARTS(2099,12,31))"
        )
