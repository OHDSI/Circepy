...
class PayerPlanPeriodSqlBuilder(CriteriaSqlBuilder[PayerPlanPeriod]):
    ...
    DEFAULT_SELECT_COLUMNS = ["ppp.person_id", "ppp.payer_plan_period_id"]
    ...
    def get_criteria_sql_with_options(
        self, criteria: PayerPlanPeriod, options: Optional[BuilderOptions]
    ) -> str:
    ...
    def resolve_where_clauses(
        ...
        # gender
        if (
            criteria.gender is not None
            and hasattr(criteria.gender, "__len__")
        ...
        if criteria.payer_source_concept is not None:
            where_clauses.append(
                f"C.payer_source_concept_id in (SELECT concept_id from #Codesets where codeset_id = {criteria.payer_source_concept})"
            )
        ...
        if criteria.sponsor_source_concept is not None:
            where_clauses.append(
                f"C.sponsor_source_concept_id in (SELECT concept_id from #Codesets where codeset_id = {criteria.sponsor_source_concept})"
            )

        # stop reason SourceConcept
        if criteria.stop_reason_source_concept is not None:
            where_clauses.append(
                f"C.stop_reason_source_concept_id in (SELECT concept_id from #Codesets where codeset_id = {criteria.stop_reason_source_concept})"
            )
