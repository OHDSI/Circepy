...
"""
    ...
    def embed_ordinal_expression(
        ...
        if criteria.first is not None and criteria.first:
            ...
            query = query.replace(
                "@ordinalExpression",
                ", row_number() over (PARTITION BY co.person_id ORDER BY co.condition_start_date, co.condition_occurrence_id) as ordinal",
            )
        else:
    ...
    def resolve_where_clauses(
        self, criteria: ConditionOccurrence, options: BuilderOptions | None = None
    ) -> list[str]:
        ...
        if criteria.condition_type_cs is not None:
            codeset_clause = BuilderUtils.get_codeset_in_expression(
                ...
                criteria.condition_type_cs.is_exclusion,
            )
            if codeset_clause:
        ...
        if criteria.condition_status_cs is not None:
            codeset_clause = BuilderUtils.get_codeset_in_expression(
                ...
                criteria.condition_status_cs.is_exclusion,
            )
            if codeset_clause:
                where_clauses.append(codeset_clause)
