...
def _apply_numeric_predicate(expr, predicate: NumericRangePredicate):
    ...
    if op in {"bt", "between"}:
        if extent is None:
            raise CompilationError(
                "Ibis executor compilation error: person numeric range 'between' requires an extent value."
            )
    ...
    raise CompilationError(
        f"Ibis executor compilation error: unsupported person numeric range op {predicate.op!r}."
    )
