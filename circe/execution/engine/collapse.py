...
def _collapse_era(intervals, era_pad: int):
    ...
    marked = with_prev.mutate(
        _is_new_group=ibis.ifelse(
            with_prev._prev_max_padded_end.isnull() | (with_prev._prev_max_padded_end < with_prev.start_date),
    ...
    return collapsed.select(
        ...
        (collapsed._max_padded_end - ibis.interval(days=int(era_pad))).cast("date").name(END_DATE),
    )
