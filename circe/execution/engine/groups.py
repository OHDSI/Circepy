...
def _evaluate_group(
    ...
    for correlated in group.criteria:
        ...
        child_results.append(correlated_matches.mutate(index_id=ibis.literal(index_id, type="int64")))
        index_id += 1

    for demographic in group.demographics:
        demographic_matches = demographic_match_keys(index_events, demographic, ctx)
        child_results.append(demographic_matches.mutate(index_id=ibis.literal(index_id, type="int64")))
        index_id += 1

    for child_group in group.groups:
        child_group_matches = _evaluate_group(index_events, child_group, ctx)
        child_results.append(child_group_matches.mutate(index_id=ibis.literal(index_id, type="int64")))
        index_id += 1
