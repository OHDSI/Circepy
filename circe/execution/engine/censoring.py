...
def apply_censoring(events, criteria, window, ctx):
    ...
    valid = joined.filter(
        (joined.censor_start_date >= joined.start_date) & (joined.censor_start_date <= joined.op_end_date)
    )
    ...
    merged = with_bounds.left_join(
        censor_min,
        predicates=[
            (with_bounds.person_id == censor_min.person_id) & (with_bounds.event_id == censor_min.event_id)
        ],
    )
