...
def literal_column_relation(
    ...
    *,
    ...
    dummy = ops.DummyTable(
        values=FrozenOrderedDict({"__values__": literal_array.op()})
    ).to_expr()
    ...
    *,
    ...
    return ops.DummyTable(
        values=FrozenOrderedDict(
            {
                column: _typed_literal(row.get(column), dtype=dtype).op()
    ...
    *,
    ...
    if not rows:
        empty_row = _single_row_relation(
            {column: None for column in schema},
            schema=schema,
        )
...
__all__ = ["literal_column_relation", "literal_rows_relation"]
