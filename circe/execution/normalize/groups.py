...
def _normalize_demographic(
    ...
    return NormalizedDemographicCriteria(
        ...
        ethnicity_codeset_id=(
            ...
            if demographic.ethnicity_cs
            and demographic.ethnicity_cs.codeset_id is not None
            else None
        ),
...
def normalize_criteria_group(
    ...
    return NormalizedCriteriaGroup(
        ...
        criteria=tuple(
            _normalize_correlated_criteria(correlated)
            for correlated in (group.criteria_list or [])
        ),
        groups=tuple(normalized_children),
        demographics=tuple(
            _normalize_demographic(demographic)
