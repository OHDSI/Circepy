...
def create_spm_rubric():
    ...
    with EvaluationBuilder("Secondary Primary Malignancy Evaluation") as ev:
        ...
        primary_cancer = ev.concept_set("Primary Cancer", 443392)
        ...
        ev.add_rule("History of Prior Primary Cancer", weight=5.0, category="Baseline").condition(
            primary_cancer
        ).at_least(1).within_days_before(3650)  # Within 10 years before index

        # Rule 2: New Primary Cancer Diagnosis at Index
        # The anchor point (index_date) should be the suspected second primary.
        ev.add_rule("Index Primary Cancer Diagnosis", weight=10.0, category="Evidence").condition(
            primary_cancer
        ).at_least(1).within_days_before(0)  # On the index date

        # Rule 3: Long interval between cancers (supports independence)
        # Using a gap of at least 2 years before the index date.
        with ev.rule(
            "Long Disease-Free Interval (>2yrs)", weight=8.0, category="Clinical Confidence"
        ) as rule:
        ...
        ev.add_rule("Genetic Susceptibility Marker", weight=12.0, category="Biomarker").observation(
            genetic_risk
        ).anytime_before()
        ...
        ev.add_rule("Histology Confirmed Distinct", weight=10.0, category="Pathology").observation(
            histology
        ...
        ev.add_rule(
            "Contralateral Organ Involvement", weight=15.0, category="Clinical Confidence"
        ).observation(contralateral).within_days_before(30)
        ...
        ev.add_rule("Radical Surgical Intent", weight=12.0, category="Treatment").procedure(
            radical_surgery
        ...
        with (
            ev.rule("Any Sex-Specific Marker Elevation", weight=8.0, category="Laboratory") as rule,
            rule.any_of() as group,
        ):
...
evaluation_sql = query_builder.build_query(
    ...
    cdm_schema="optum_ehr.cdm_optum_ehr_v3765",
    vocabulary_schema="optum_ehr.cdm_optum_ehr_v3765",  # Assuming vocab is in same schema or accessible
    results_schema="scratch.scratch_jgilber2",
