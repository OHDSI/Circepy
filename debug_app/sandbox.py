    ...
    for line in code.split('\n'):
        ...
        if match:
            ...
            if not any(module.startswith(allowed) for allowed in allowed_modules):
                return False, f"Import '{module}' is not allowed. Only 'circe.cohort_builder' and 'circe.vocabulary' imports are permitted."
...
def execute_cohort_code(code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        json_output = json.dumps(
            cohort_expression.model_dump(exclude_none=True, by_alias=True),
            indent=2
        )
