...
def normalize_sql(sql: str) -> str:
    ...
    sql = re.sub(r'\{[^}]*\}\?\{', '', sql)
    sql = re.sub(r'\}', ' ', sql)
    sql = re.sub(r'--\s*the matching group.*?inclusion_rule_mask\s+=\s+power\(.*?\)\s*-\s*1\)\)\s*results', ') results', sql, flags=re.DOTALL)
    ...
    keywords = [
        'select', 'from', 'inner join', 'left join', 'right join', 'join', 
        'where', 'group by', 'order by', 'having', 'limit', 'union', 'with', 'intersect', 'except'
    ]
    for kw in keywords:
...
def generate_reference_with_r(json_content: str) -> dict:
    ...
    if not os.path.exists(r_script_path):
         return {"error": f"R script not found at {r_script_path}", "sql": "", "markdown": ""}
