...
class SkillGenerator:
    ...
    def generate_markdown(self) -> str:
        ...
        md.append("")
        md.append("### 1. Demographic Methods Accept Multiple Values, NOT Lists")
        md.append("")
        md.append("❌ **WRONG**:")
        md.append("```python")
        md.append(".require_gender([8507, 8532])  # ERROR! Pydantic validation error")
        md.append(".require_race([8516])           # ERROR!")
        md.append("```")
        md.append("")
        md.append("✅ **CORRECT**:")
        md.append("```python")
        md.append(".require_gender(8507, 8532)     # Multiple values as separate arguments")
        md.append(".require_race(8516)             # Single value")
        md.append(".require_ethnicity(38003563, 38003564)  # Multiple values unpacked")
        md.append("```")
        md.append("")
        md.append("### 2. Time Windows Are ONLY on Query Builders")
        md.append("")
        md.append("Time window methods (`.anytime_before()`, `.within_days_after()`, etc.) exist ONLY on query builders returned by `.require_X()` and `.exclude_X()` methods.")
        md.append("")
        md.append("❌ **WRONG**:")
        md.append("```python")
        md.append(".with_condition(1).anytime_after()  # ERROR! CohortWithCriteria has no time windows")
        md.append("```")
        md.append("")
        md.append("✅ **CORRECT**:")
        md.append("```python")
        md.append(".with_condition(1)              # Returns CohortWithCriteria")
        md.append(".require_drug(2).anytime_after()  # .require_drug() returns DrugQuery (has time windows!)")
        md.append("```")
        md.append("")
        md.append("**API Flow**: `CohortWithCriteria.require_X()` → `QueryBuilder` (with time windows) → `.time_window()` → back to `CohortWithCriteria`")
        md.append("")
        ...
        for method in sorted(self.entry_methods, key=lambda m: m.name):
            if method.name in ['first_occurrence', 'with_observation', 'min_age', 'max_age']:
        ...
        for method in sorted(self.criteria_methods, key=lambda m: m.name):
            if method.name in ['require_any_of', 'require_all_of', 'require_at_least_of', 'exclude_any_of']:
                md.append(f"### `.{method.signature}`")
                if method.docstring:
